...
| [Entra application proxy](docs/app-proxy.md) | Design **and a step-by-step deployment runbook** for the optional `useAppProxy` mode: replace the public LB with Microsoft Entra application proxy (outbound-only connector, Entra pre-auth, vanity FQDN + Let's Encrypt cert). |
